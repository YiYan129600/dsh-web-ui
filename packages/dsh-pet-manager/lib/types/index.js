...
export class PetManagerService extends Service {
    ...
    runtimeEnabled(provider) {
        ...
        if (!settings)
            return true;
        const descriptor = settings.describe({ redactSecrets: true }).find((d) => String(d.ns) === provider.settingsNamespace);
    ...
    async setEnabled(entryId, enabled) {
        ...
        if (provider.toggleMode === 'runtime') {
            ...
            try {
                ...
                await settings.mutate(settingsNamespace(provider.settingsNamespace), fields.map((field) => ({ op: 'set', path: [field], value: enabled })));
            }
            catch (error) {
    ...
    async describe(namespace) {
        ...
        if (!descriptor)
            return { ok: false, error: 'namespace-not-registered' };
